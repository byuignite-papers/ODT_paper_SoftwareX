...
The ODT package consists primarily of an object-oriented C++ code responsible for running flow simulation cases and generating data. The package also contains auxiliary data processing and visualization tools, written mostly in Python. Within the main download package, several directories organize the code:
\begin{itemize} \itemsep0em
	\item \texttt{source}: ODT source code
	\item \texttt{build}: compiles and builds source code via CMake
	\item \texttt{run}: ODT executable and run scripts
	\item \texttt{data}: data generated by source code
	\item \texttt{input}: input files used to specify case variables
	\item \texttt{post}: data post-processing and visualization scripts
	\item \texttt{doc}: code documentation generated by Doxygen
\end{itemize}



% source code stuff
In an individual simulation, variables live within in the \texttt{domain} class; wherever these variables are used within the code, pointers refer back to these locations. Key quantities are referred to as line variables, and their individual properties and functions exist within child classes of the \texttt{domainvariables} parent class. 

The system of nonlinear ODEs is solved using CVODE \cite{Hindmarsh_2020} and user input files are processed with YAML \cite{Beder_2008}, both of which are installed locally during the ODT build process. For reacting flow cases, chemical kinetics and transport are handled by Cantera \cite{Goodwin_2018}, which must be previously installed by the user. 

\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{../figures/odt_flow_chart/odt_flow_chart.png} 
	\caption{Diagram of high-level ODT code structure}
	\label{fig:flowchart}
\end{figure}

Figure \ref{fig:flowchart} illustrates the basic outline of the ODT code structure.

% You can talk about the basic structure: domain.cc is the main object that holds pointers to everything else. There are back pointers to the domain, so that everyone can see everyone else, as needed. The key quantities are line variables (arrays stored at each point). These are the domainvariables. The domain class has a vector of pointers to these, and also pointers to specific variables to access them by name. This enables both generic looping over variables to have each variable compute, for example, its source term, and also allow specific access to needed variables like the grid positions, etc. The variables are either transported or not. The variables needed for a given simulation are set up in the domaincases class. In addition, there are general parameters. You could discuss the main classes, or maybe show a class flow diagram. These can be easily generated, and doxygen also does some of this.
...
%\label{sub:workflow}
%
%Every ODT simulation shares common steps to a successful run. 
%\begin{enumerate}
%	\item Build and compile the code. 
%	\item Choose a case type and chemical mechanism (when required) and alter the appropriate input file(s). 
%	\item Within the \texttt{run} directory, alter the run script to be used with a case name and other required information.
%	\item Run the case from the \texttt{run} directory using the chosen run script.
%	\item Run the appropriate post-processing scripts, located in the \texttt{post} directory, on the raw data for interpretation and visualization.
%\end{enumerate}
